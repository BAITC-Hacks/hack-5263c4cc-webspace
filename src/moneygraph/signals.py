...
from .engine import Analysis, _day, _number
...
class SignalAnalysis:
    ...
    def __init__(self, analysis: Analysis):
        ...
        for source in analysis._transactions:
    ...
    def _require(self, gid: int) -> dict[str, Any]:
        node = self.analysis._records.get(gid)
        if node is None:
            raise KeyError(gid)
        return node
    ...
    def _temporal(self, gid: int, node: dict[str, Any]) -> dict[str, Any]:
        daily = self.analysis._daily.get(gid, {})
    ...
    def _routes(self, gid: int) -> tuple[list[dict[str, Any]], bool]:
        routes, examined = [], 0
        incoming = sorted(src for src in self.G.predecessors(gid) if src != gid)
        outgoing = sorted(dst for dst in self.G.successors(gid) if dst != gid)
    ...
    def _cycles(self, gid: int) -> tuple[list[dict[str, Any]], bool]:
        ...
        def walk(path: list[int]) -> None:
            ...
            if limited:
                return
            for target in sorted(self.G.successors(path[-1])):
                steps += 1
        ...
        for path in paths[:MAX_CYCLES]:
            edges = [{"src": src, "dst": dst, "sum_kzt": _number(self.G[src][dst]["sum_kzt"]),
    ...
    def _anomalies(self, gid: int, node: dict[str, Any]) -> list[dict[str, Any]]:
        result = []
        peers = [r for r in self.analysis._records.values() if r["depth"] == node["depth"]]
        if len(peers) >= 10:
    ...
    def _network_metrics(self, graph: nx.DiGraph) -> dict[str, int]:
        ...
        for gid in sorted(graph):
            if self.analysis._nodes[gid]["is_seed"]:
    ...
    def resilience(self, top_n: int = 5) -> dict[str, Any]:
        ...
        removed = [r["gid"] for r in self.analysis._ranked[:top_n]]
        after_graph = self.G.copy()
        after_graph.remove_nodes_from(removed)
        before, after = self._network_metrics(self.G), self._network_metrics(after_graph)
    ...
    def collectors(self, gids: list[int], max_hops: int = 3) -> dict[str, Any]:
        ...
        gids = sorted(gids)
        traversals = {gid: nx.single_source_shortest_path(self.G, gid, cutoff=max_hops) for gid in gids}
        common = set.intersection(*(set(paths) for paths in traversals.values())) - set(gids)
        ordered = sorted(common, key=lambda gid: (-self.analysis._records[gid]["priority_score"], gid))
        items = []
        for gid in ordered[:20]:
            row = self.analysis._records[gid]
